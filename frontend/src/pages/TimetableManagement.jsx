        ...
        reader.onload = async (event) => {
            try {
                ...
                const formattedData = rawJsonData.map(row => {
                    // Xử lý ngày tháng y hệt trang trực ban
                ...
                console.error(error);
                alert('Có lỗi xảy ra. Vui lòng đảm bảo file Excel có đúng các cột: Tuần Ghi Nhận, Môn Học, Tuần Áp Dụng, Tuần Kết Thúc, Ngày Thông Báo, Ghi Chú');
            } finally {
                e.target.value = null;
            }
        };
                    ...
                    <table className="w-full text-left border-collapse min-w-[1000px]">
                        <thead className="sticky top-0 z-10 bg-[linear-gradient(135deg,#2563eb_0%,#1e40af_100%)] text-white shadow-md">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-base font-medium whitespace-nowrap">Tuần Ghi Nhận</th>
                                <th className="px-6 py-4 text-base font-medium whitespace-nowrap">Môn Học</th>
                                <th className="px-6 py-4 text-base font-medium whitespace-nowrap">Áp Dụng Từ Tuần</th>
                                <th className="px-6 py-4 text-base font-medium whitespace-nowrap">Đến Tuần</th>
                                <th className="px-6 py-4 text-base font-medium whitespace-nowrap">Ngày Thông Báo</th>
                            ...
                            {filteredChanges.length > 0 ? (
                                filteredChanges.map((item, index) => (
                                    <tr key={item._id} className="hover:bg-blue-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-base font-[450] text-blue-700 font-semibold">Tuần {item.week}</td>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input name="week" type="number" value={formData.week} onChange={handleChange} required className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-[#2453c9] focus:ring-1 focus:ring-[#2453c9] transition-colors" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1.5">Môn học thay đổi</label>
                                    <input name="subject" value={formData.subject} onChange={handleChange} required className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-[#2453c9] focus:ring-1 focus:ring-[#2453c9] transition-colors" />
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input name="applyWeek" type="number" value={formData.applyWeek} onChange={handleChange} required className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-[#2453c9] focus:ring-1 focus:ring-[#2453c9] transition-colors" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1.5">Tuần kết thúc</label>
                                    <input name="endWeek" type="number" value={formData.endWeek} onChange={handleChange} required className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-[#2453c9] focus:ring-1 focus:ring-[#2453c9] transition-colors" />
                                </div>
                            </div>

                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1.5">Ngày thông báo cho GV/HS</label>
                                <input type="date" name="notificationDate" value={formData.notificationDate} onChange={handleChange} required className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-[#2453c9] focus:ring-1 focus:ring-[#2453c9] transition-colors" />
                            </div>

                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1.5">Ghi chú thêm</label>
                                <textarea name="notes" value={formData.notes} onChange={handleChange} rows="2" className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-[#2453c9] focus:ring-1 focus:ring-[#2453c9] transition-colors resize-none" />
                            </div>
